...
/* linP_vel_ = tf::Vector3(0.0, 0.0, 0.0); //previous step vel
    linP_pos_ = tf::Vector3(0.0, 0.0, 0.0); //previous step pos */
  
  linP_pos_.x = 0; linP_pos_.y = 0; linP_pos_.z = 0; //previous step pos
  linP_vel_.x = 0; linP_vel_.y=0; linP_vel_.z=0; //previous step vel
...
geometry_msgs::Vector3 linW_pos;
...
{
  lin_vel.x = 0; lin_vel.y=0; lin_vel.z=0;
}
else 
{
...
// Rotate body accelerations to Earth frame
    ...
    linW_acc.x =  (q1*q1-q2*q2-q3*q3+q0*q0)*lin_acc.x + 2*(q1*q2+q3*q0)*lin_acc.y + 2*(q1*q3-q2*q4)*lin_acc.z;
    linW_acc.y = 2*(q1*q2-q3*q0)*lin_acc.x + (-q1*q1+q2*q2-q3*q3+q4*q4)*lin_acc.y + 2*(q2*q3+q1*q0)*lin_acc.z;
    linW_acc.z = 2*(q1*q3+q2*q0)*lin_acc.x + 2*(q2*q3-q1*q0)*lin_acc.y + (-q1*q1-q2*q2+q3*q3+q0*q0)*lin_acc.z - 9.81;

// Compute translational velocities
// Integrate acceleration to yield velocity
    lin_vel.x = linP_vel_.x + linW_acc.x*dt;
    lin_vel.y = linP_vel_.y + linW_acc.y*dt;
    lin_vel.z = linP_vel_.z + linW_acc.z*dt;
    linP_vel_.x = lin_vel.x;
    linP_vel_.y = lin_vel.y;
    linP_vel_.z = lin_vel.z;
    
// Compute translational position
// Integrate velocity to yield position
    linW_pos.x = linP_pos_.x + lin_vel.x*dt;
    linW_pos.y = linP_pos_.y + lin_vel.y*dt;
    linW_pos.z = linP_pos_.z + lin_vel.z*dt;
    linP_pos.x = linW_pos.x;
    linP_pos.y = linW_pos.y;
    linP_pos.z = linW_pos.z;
}
}
