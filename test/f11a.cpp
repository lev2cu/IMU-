  ...
  q0(1.0), q1(0.0), q2(0.0), q3(0.0)
{
  ...
  imu_publisher_ = nh_.advertise<sensor_msgs::Imu>(
    "imu/data", 5);
//imu_pose_ = nh_.advertise<tf::Vector3>("pose",1);
...
void ImuFilter::imuCallback(const ImuMsg::ConstPtr& imu_msg_raw)
{
  ...
  if (!initialized_)
  {
    ...
    last_time_ = time;
    initialized_ = true;
    linP_vel_ = geometry_msgs::Vector3(0.0, 0.0, 0.0); //previous step vel
    linP_pos_ = geometry_msgs::Vector3(0.0, 0.0, 0.0); //previous step pos
  }
...
{
  lin_vel = geometry_msgs::Vector3(0.0, 0.0, 0.0);
}
else 
{
// Compute translational accelerations
// Rotate body accelerations to Earth frame     
    linW_acc.x =  (q1*q1-q2*q2-q3*q3+q0*q0)*lin_acc.x + 2*(q1*q2+q3*q0)*lin_acc.y + 2*(q1*q3-q2*q0)*lin_acc.z;
    linW_acc.y = 2*(q1*q2-q3*q0)*lin_acc.x + (-q1*q1+q2*q2-q3*q3+q0*q0)*lin_acc.y + 2*(q2*q3+q1*q0)*lin_acc.z;
...
// Integrate velocity to yield position
    ...
    linP_pos_ = linW_pos;
//ROS_INFO("pose:%f", linW_pos.Vector.x);
}
}
